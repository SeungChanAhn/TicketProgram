...
int main() {
    ...
    do {
        ...
        orderList[orderCount][2] = realAge;
        orderList[orderCount][3] = price;
        orderList[orderCount][4] = alwaysPrefer;
        orderCount++;
    ...
    printf("\n\n******************** 결제 정보 ********************\n");
    printf("티켓\t시간\t만나이\t가격\t우대사항\n");
    for (int i = 0; i < orderCount; i++) {
        printf("%d\t%d\t%d\t%d\t\t%d\n", orderList[i][0], orderList[i][1], orderList[i][2], orderList[i][3], orderList[i][4]);
    }
    printf("-------------------------------------------------------\n");
    printf("결제 금액\t\t\t\t%d원\n\n", totalprice);
}
